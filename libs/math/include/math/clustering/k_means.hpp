...
private:
  ...
  void KMeansPPInitialisation(ArrayType const &data)
  {
    ...
    std::vector<DataType> weights(n_points_);   // weight for choosing each data point
    std::vector<DataType> interval(n_points_);  // interval for defining random distribution
